...
import { Layout } from './LandownerDashboard'; // Assuming this provides the layout component
...
  phone: string | null;
  role: string | null;
  profileImage: string | null; // Renamed to match server property
  createdAt: string;
}
...
        // Corrected API endpoint, no email query parameter is needed
...
        setError(
          err.response?.data?.message || "Failed to load profile. Please try again later."
        );
      } finally {
        setLoading(false);
      }
...
            <p className="text-gray-700 text-lg font-medium">No profile data available.</p>
          </div>
        </div>
      </Layout>
    );
...
              User Profile
            </h2>
            <p className="text-md text-gray-500 mt-2">Your personal details at a glance</p>
          </div>
...
            {profile.profileImage ? ( // Corrected property name from 'profilePic' to 'profileImage'
              <img
...
                    />
                  </div>
                  <ProfileDetail label="Role" value={profile.role || "Not assigned"} />
                  <ProfileDetail 
                    label="Joined" 
...
              ) : (
                <>
                  <ProfileDetail label="Name" value={profile.name || "Not set"} />
                  <ProfileDetail label="Email" value={profile.email} />
                  <ProfileDetail label="Phone" value={profile.phone || "Not set"} />
                  <ProfileDetail label="Role" value={profile.role || "Not assigned"} />
                  <ProfileDetail 
                    label="Joined" 
                    value={new Date(profile.createdAt).toLocaleDateString("en-IN", {
...
const ProfileEditField: React.FC<{ label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; disabled?: boolean; }> = ({ label, name, value, onChange, disabled = false }) => (
  <div className="w-full">
...
      onChange={onChange}
      disabled={disabled}
      className={`mt-1 block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 ${disabled ? "bg-gray-200 cursor-not-allowed" : "bg-white"}`}
    />
  </div>
);
