    ...
    const fetchProjects = async () => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/projects/funded`,
          {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6 mb-4 sm:mb-6">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate">Active Projects</h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1 line-clamp-2">Monitor successfully funded agricultural initiatives currently in execution</p>
            </div>
          </div>
        </div>
        ...
        {projects.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm border p-8 sm:p-12 text-center">
            ...
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">No Active Projects</h3>
            <p className="text-sm sm:text-base text-gray-600">There are currently no active funded projects in progress.</p>
          </div>
        ) : (
          ...
          <div className="mt-4 sm:mt-6 text-center">
            <div className="inline-flex items-center bg-white rounded-lg px-4 sm:px-6 py-2.5 sm:py-3 border shadow-sm">
              ...
              <span className="text-gray-600 text-xs sm:text-sm font-medium">
                Tracking <span className="text-gray-900 font-semibold">{projects.length}</span> active project{projects.length !== 1 ? "s" : ""}
              </span>
            </div>
          </div>
        )}
