...
const AcceptedLeases: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAcceptedLeases = async () => {
      try {
        ...
        const res = await fetch("https://agricorus.duckdns.org/api/farmer/leases/accepted", {
  ...
  const handlePayment = async (leaseId: string) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://agricorus.duckdns.org/api/payments/order/${leaseId}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const options = {
        ...
        handler: async function (response: any) {
          const verifyRes = await fetch(`https://agricorus.duckdns.org/api/payments/verify`, {
            method: "POST",
