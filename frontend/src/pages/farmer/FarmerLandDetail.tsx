...
import { MapPin, Droplet, Route, Ruler, Phone, Mail, Calendar, Clock, Server, Cloud, Shield, Cpu } from "lucide-react";
...
const FarmerLandDetail: React.FC = () => {
  ...
  const techIcons = [Server, Cloud, Shield, Cpu];
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-300 font-medium font-poppins text-lg">Loading land details...</p>
          <p className="text-gray-500 text-sm mt-2 font-inter">Accessing agricultural network</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="text-center">
          ...
          <h3 className="text-2xl font-bold text-white font-poppins uppercase tracking-wide mb-4">Land Not Found</h3>
          <p className="text-gray-400 font-inter text-lg">The requested land details could not be loaded from our network.</p>
        </div>
      </div>
    );
  }

  const randomTechIcon = techIcons[Math.floor(Math.random() * techIcons.length)];
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="relative z-10">
            <div className="flex items-center mb-4">
              ...
              <div>
                ...
                <p className="text-gray-300 font-inter text-lg mt-2">Complete technical specifications for this agricultural land</p>
              </div>
            </div>
            ...
            <div className="inline-flex items-center gap-2 bg-gray-800/50 px-4 py-2 rounded-full border border-white/10">
              <div className={`w-2 h-2 rounded-full ${
                land.status === 'available' ? 'bg-green-500 animate-pulse' : 
                land.status === 'leased' ? 'bg-red-500' : 'bg-yellow-500'
              }`}></div>
        ...
        >
          {land.landPhotos && land.landPhotos.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-48 bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-2xl border border-white/10 backdrop-blur-sm">
              ...
              <span className="ml-4 text-gray-400 font-inter">No images available in database</span>
            </div>
          )}
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { icon: MapPin, label: "Location Address", value: land.location.address, color: "from-blue-500 to-cyan-500" },
                { icon: Droplet, label: "Water Source", value: land.waterSource || "Not specified", color: "from-blue-500 to-cyan-500" },
                { icon: Ruler, label: "Land Size", value: `${land.sizeInAcres || "N/A"} acres`, color: "from-purple-500 to-pink-500" },
                { icon: Route, label: "Accessibility", value: land.accessibility || "Not specified", color: "from-green-500 to-emerald-500" },
                { icon: Calendar, label: "Lease Duration", value: `${land.leaseDurationMonths} months`, color: "from-orange-500 to-red-500" },
                { 
                  icon: Shield, 
                  label: "Soil Type", 
                  value: land.soilType, 
                ...
                >
                  <div className="flex items-center mb-3">
                    ...
                    <span className="text-sm text-gray-400 uppercase tracking-wide font-poppins">{item.label}</span>
                  </div>
                  <p className={`text-lg font-semibold font-inter ${
                    item.special ? "bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent" : "text-white"
                  }`}>
                    {item.value}
                  </p>
                </motion.div>
          ...
          >
            ...
            <div className="mb-6 p-4 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-xl border border-blue-500/20">
              <div className="flex justify-between items-center mb-3">
                <span className="text-sm text-gray-300 uppercase tracking-wide font-poppins">Monthly Rate</span>
                <span className="text-2xl font-bold text-white font-poppins">
                  ₹{land.leasePricePerMonth.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-400">Total for {land.leaseDurationMonths} months</span>
            ...
            {land.owner && (
              <div className="mb-6 p-4 bg-gray-800/30 rounded-xl border border-white/5">
                <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3 font-poppins">Owner Contact</h3>
                <div className="space-y-3">
                  <div className="flex items-center text-sm text-gray-400 hover:text-white transition-colors duration-300">
                    ...
                    <span className="font-inter">{land.owner.email || "N/A"}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-400 hover:text-white transition-colors duration-300">
                    <Phone className="w-4 h-4 mr-3 text-green-400" />
                    <span className="font-inter">{land.owner.phone || "N/A"}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mt-6 pt-4 border-t border-white/10">
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="bg-gray-800/30 p-2 rounded-lg">
                  <div className="text-xs text-gray-400 uppercase tracking-wide font-poppins">Size</div>
                  <div className="text-sm font-semibold text-white">{land.sizeInAcres || "N/A"} acres</div>
                </div>
                <div className="bg-gray-800/30 p-2 rounded-lg">
                  <div className="text-xs text-gray-400 uppercase tracking-wide font-poppins">Duration</div>
                  <div className="text-sm font-semibold text-white">{land.leaseDurationMonths} mo</div>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              <img src={selectedImage} alt="Enlarged view" className="w-full h-full object-contain rounded-xl" />
            </motion.div>
          </motion.div>
        )}
      </div>
