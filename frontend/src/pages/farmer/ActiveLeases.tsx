...
const ActiveLeases: React.FC = () => {
  ...
  const getImageUrl = (path: string) => path?.startsWith('http') ? path : `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/${path}`;
  ...
  const fetchLeases = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/farmer/leases/active`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handlePayment = async (leaseId: string) => {
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/payments/order/${leaseId}`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
      ...
      new window.Razorpay({
        ...
        handler: async (r: any) => {
          const v = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/payments/verify`, {
  ...
  const submitDispute = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/disputes`, {
  ...
  return (
    <div className="max-w-7xl mx-auto p-3 sm:p-4 lg:p-6">
      ...
      <div className="relative mb-6 sm:mb-8">
        ...
        {/* Icon Badge */}
        <div className="absolute -bottom-8 sm:-bottom-10 lg:-bottom-12 left-1/2 -translate-x-1/2">
          <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 p-1 shadow-xl">
            <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
              <DocumentTextIcon className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-emerald-600" />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Title Section */}
      <div className="text-center mt-12 sm:mt-14 lg:mt-16 mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Active Leases</h1>
        <p className="text-sm sm:text-base text-gray-500 mt-2">Manage and monitor agricultural lease operations</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
        {[
          ...
          <div key={index} className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-4 lg:p-6 hover:shadow-lg transition-all group">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
              <div className="flex-1 min-w-0">
                <p className="text-gray-500 text-xs sm:text-sm mb-1 truncate">{stat.label}</p>
                <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800">{stat.value}</p>
              </div>
              <div className={`w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-lg sm:rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform flex-shrink-0`}>
