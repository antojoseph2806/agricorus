...
const FarmerProfile: React.FC = () => {
  const [user, setUser] = useState<FarmerProfileData | null>(null);
  ...
  const BASE_URL = `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}`;
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        setLoading(true);
        const { data } = await axios.get<FarmerProfileData>(`${BASE_URL}/api/profile`, {
        ...
        if (data.profileImage) {
          setPreview(`${BASE_URL}${data.profileImage}`);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.put<FarmerProfileData>(`${BASE_URL}/api/profile`, submitData, {
        headers: {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {saveSuccess && (
          <div className="fixed top-4 right-4 left-4 sm:left-auto z-50 animate-slide-in">
            <div className="flex items-center gap-2 sm:gap-3 bg-emerald-500 text-white px-4 sm:px-6 py-3 sm:py-4 rounded-xl shadow-lg">
        ...
        <div className="relative mb-8">
          ...
          <div className="absolute -bottom-12 sm:-bottom-16 left-1/2 -translate-x-1/2">
            <div className="relative group">
              <div className="w-24 h-24 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 p-1 shadow-xl">
                <img
                  src={preview || `https://via.placeholder.com/150/10B981/FFFFFF?text=${user.name?.charAt(0) || "F"}`}
                  alt="Profile"
                  className="w-full h-full object-cover rounded-full bg-white"
                />
              </div>
              {isEditing && (
                <label className="absolute bottom-1 right-1 sm:bottom-2 sm:right-2 w-8 h-8 sm:w-10 sm:h-10 bg-emerald-500 rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:bg-emerald-600 transition-all hover:scale-110">
        ...
        <div className="text-center mt-16 sm:mt-20 mb-6 sm:mb-8 px-4">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 break-words">{user.name || "Farmer"}</h1>
          <div className="inline-flex items-center gap-2 mt-3 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-full border border-emerald-100">
            ...
            <span className="text-sm sm:text-base text-emerald-700 font-medium capitalize">{user.role || "Farmer"}</span>
          </div>
        </div>

        {/* Profile Card */}
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          <div className="px-4 sm:px-8 py-4 sm:py-6 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            <div>
          ...
          <div className="p-4 sm:p-8">
            {isEditing ? (
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={user.email}
                      disabled
                      className="w-full px-4 sm:px-5 py-3 sm:py-4 bg-gray-100 border-2 border-gray-100 rounded-xl text-gray-500 cursor-not-allowed text-sm sm:text-base pr-16"
                    />
                ...
                <div>
                  <label className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                    <Phone className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500" />
                    Phone Number
                  </label>
                  <input
                    type="tel"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      disabled
                      className="w-full px-4 sm:px-5 py-3 sm:py-4 bg-gray-100 border-2 border-gray-100 rounded-xl text-gray-500 cursor-not-allowed capitalize text-sm sm:text-base pr-16"
                    />
                    <span className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-xs bg-gray-200 text-gray-500 px-2 py-1 rounded-md">
                      Locked
                    </span>
              ...
              <div className="grid gap-4 sm:gap-6">
                ...
                <div className="flex items-center gap-3 sm:gap-4 p-4 sm:p-5 bg-gradient-to-r from-gray-50 to-white rounded-xl sm:rounded-2xl border border-gray-100 hover:border-emerald-200 hover:shadow-md transition-all group">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg shadow-cyan-500/20 group-hover:scale-110 transition-transform flex-shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8">
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-100 shadow-sm hover:shadow-lg transition-all sm:col-span-2 md:col-span-1">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
            ...
            <p className="text-xl sm:text-2xl font-bold text-gray-800">Farmer</p>
