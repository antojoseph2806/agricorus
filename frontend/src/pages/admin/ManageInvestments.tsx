...
const ManageInvestments: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    setError(null);
    if (!window.confirm("CONFIRM: Are you sure you want to permanently delete this investment record?")) return;
  ...
  const handleRefund = async (id: string) => {
    ...
    if (!investment?.paymentId) return alert("This investment has no associated paymentId.");

    const amountStr = window.prompt(
      `Enter refund amount (Max: ${investment.amount.toFixed(2)}, leave blank for full refund):`
    );

    let amount: number | undefined;
    if (amountStr) {
      const parsed = parseFloat(amountStr);
      if (isNaN(parsed) || parsed <= 0 || parsed > investment.amount) return alert("Invalid amount entered.");
      amount = parsed;
    }
    ...
    try {
      ...
      alert(`Refund successful: ${res.data.message} (Razorpay ID: ${res.data.refund.id})`);
      fetchAdminData(); // Refresh data to update total funding stats
    } catch (err: any) {
      console.error("Error refunding investment:", err.response?.data || err.message);
      setError(err.response?.data?.msg || "Refund failed.");
      alert(`Refund failed: ${err.response?.data?.msg || err.message}`);
    }
  };
                        ...
                        <td style={{...cellStyle, textAlign: "center"}}>
                          <button 
                            ...
                            style={{
                              ...
                              marginRight: "10px",
                              opacity: inv.paymentId ? 1 : 0.5,
                              background: inv.paymentId ? "rgba(255,195,0,0.2)" : "rgba(255,255,255,0.1)",
                              border: inv.paymentId ? "1px solid rgba(255,195,0,0.3)" : "1px solid rgba(255,255,255,0.2)"
                            }}
                            disabled={!inv.paymentId}
                            onMouseEnter={(e) => {
                              if (inv.paymentId) {
                                e.currentTarget.style.background = "rgba(255,195,0,0.3)";
                          ...
                          <button 
                            ...
                            onMouseEnter={(e) => {
                              ...
                              e.currentTarget.style.boxShadow = "0 0 20px rgba(255, 82, 82, 0.5)";
                              e.currentTarget.style.transform = "scale(1.05)";
                            }}
                            onMouseLeave={(e) => {
...
// ---------------------------------------------------------------------------
// 🌟 New: Component to display investment statistics
// ---------------------------------------------------------------------------
        ...
        <div style={{ marginBottom: '30px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', marginBottom: '30px' }}>
                ...
                <div 
                    ...
                    onMouseEnter={(e) => {
                        e.currentTarget.style.transform = statBoxHoverEffect.transform;
                        e.currentTarget.style.boxShadow = statBoxHoverEffect.boxShadow;
                    }}
                    onMouseLeave={(e) => {
                        e.currentTarget.style.transform = "scale(1)";
