...
const ProductPerformance: React.FC = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/admin/sales-analytics/products`, {
  ...
  const downloadProductReport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/admin/sales-analytics/download?type=products`, {
        headers: { Authorization: `Bearer ${token}` },
      });
