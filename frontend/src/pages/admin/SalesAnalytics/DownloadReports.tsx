...
const DownloadReports: React.FC = () => {
  ...
  const generateReport = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/sales-analytics/generate-report', {
        method: 'POST',
        headers: {
  ...
  const downloadExistingReport = async (requestId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/admin/sales-analytics/download-report/${requestId}`, {
