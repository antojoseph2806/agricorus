...
const MonthlyReports: React.FC = () => {
  ...
  const fetchMonthlyData = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/admin/sales-analytics/monthly?month=${selectedMonth + 1}&year=${selectedYear}`,
        {
  ...
  const downloadMonthlyReport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/admin/sales-analytics/download?type=monthly&month=${selectedMonth + 1}&year=${selectedYear}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
