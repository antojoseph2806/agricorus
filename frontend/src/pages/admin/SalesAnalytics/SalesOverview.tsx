...
const SalesOverview: React.FC = () => {
  ...
  const fetchSalesData = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}/api/admin/sales-analytics/overview?period=${selectedPeriod}`, {
  ...
  const downloadReport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}/api/admin/sales-analytics/download?type=overview&period=${selectedPeriod}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
