...
const PaymentHistory: React.FC = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/payment-requests/my-requests`,
        {
  ...
  const cancelRequest = async (requestId: string) => {
    ...
    try {
      ...
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/payment-requests/cancel-request/${requestId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
