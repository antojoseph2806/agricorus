...
const DisputeForm: React.FC<DisputeModalProps> = ({ leaseId, onClose, onDisputeRaised }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(
        `https://agricorus.duckdns.org/api/landowner/disputes/${leaseId}`,
        payload,
        {
...
const LandownerLeaseRequests: React.FC<LeaseRequestsProps> = ({
  ...
  const fetchLeases = async () => {
    ...
    try {
      let url = "https://agricorus.duckdns.org/api/leases/owner/requests";
      if (statusFilter !== "all") {
        url = `https://agricorus.duckdns.org/api/leases/owner/requests/${statusFilter}`;
      }
  ...
  const handleAction = async (leaseId: string, action: "accept" | "cancel") => {
    ...
    try {
      const token = localStorage.getItem("token");
      const url = `https://agricorus.duckdns.org/api/leases/${leaseId}/${action}`;
  ...
  const handleViewAgreement = async (lease: any) => {
    ...
    if (lease.status === 'active') {
      try {
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(`https://agricorus.duckdns.org/api/lease/${lease._id}/agreement`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
