...
const LandownerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      ...
      try {
        // Fetch lands count
        const landsResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/landowner/lands/my`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const leasesResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/landowner/leases/active`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const paymentsResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/landowner/payments/history`, {
          headers: { Authorization: `Bearer ${token}` }
        });
