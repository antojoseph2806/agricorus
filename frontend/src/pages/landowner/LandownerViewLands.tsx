...
import { DollarSign, TrendingUp, LandPlot, Eye, Trash2, MapPin, Edit, Plus, Cloud, Server, Database, Shield, Zap, Cpu } from 'lucide-react';
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {[
              ...
              { label: "Available", value: lands.filter(l => l.status === 'available').length, icon: Cloud, color: "from-green-500 to-green-600" },
              { label: "Leased", value: lands.filter(l => l.status === 'leased').length, icon: Server, color: "from-purple-500 to-purple-600" },
              { label: "Pending Review", value: lands.filter(l => !l.isApproved && !l.rejectionReason).length, icon: Cpu, color: "from-yellow-500 to-yellow-600" },
            ].map((stat, index) => (
              <div
                key={index}
                      ...
                      <div className="flex flex-col h-full">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4">
                            ...
                            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border mt-2 sm:mt-0 ${getStatusColor(land.status)}`}>
