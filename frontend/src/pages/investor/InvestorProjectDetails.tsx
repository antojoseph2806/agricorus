...
export default function InvestorProjectDetails() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        setPageLoading(true);
        const res = await axios.get(`http://localhost:5000/api/projects/investor/${id}`, {
  ...
  const handleInvest = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post("http://localhost:5000/api/project-payments/create-order",
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            await axios.post("http://localhost:5000/api/project-payments/verify",
            ...
            const updated = await axios.get(`http://localhost:5000/api/projects/investor/${project._id}`, { headers: { Authorization: `Bearer ${token}` } });
