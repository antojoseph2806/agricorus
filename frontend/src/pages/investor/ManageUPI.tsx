...
export default function ManageUPI() {
  ...
  const fetchMethods = async () => {
    const res = await axios.get("/api/payouts/", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const handleSubmit = async () => {
    try {
      if (editingId) {
        await axios.put(`/api/payouts/${editingId}`, form, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
      } else {
        await axios.post("/api/payouts/add-upi", form, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    await axios.delete(`/api/payouts/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
    fetchMethods();
  };
