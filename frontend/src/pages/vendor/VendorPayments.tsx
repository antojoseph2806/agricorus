  ...
  const fetchPayments = async () => {
    try {
      ...
      const response = await axios.get(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/payments?${params.toString()}`,
        {
  ...
  const handleRefund = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/payments/${selectedPayment._id}/refund`,
        {
          refundAmount: amount,
