...
const ProductList = () => {
  ...
  const fetchKYCStatus = async () => {
    try {
      ...
      const res = await axios.get("https://agricorus.duckdns.org/api/vendor/profile/status", {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const url =
        filter === "all"
          ? "https://agricorus.duckdns.org/api/vendor/products"
          : `https://agricorus.duckdns.org/api/vendor/products?isActive=${filter === "active"}`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://agricorus.duckdns.org/api/vendor/products/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleToggleActive = async (product: Product) => {
    ...
    try {
      ...
      const res = await axios.patch(
        `https://agricorus.duckdns.org/api/vendor/products/${product._id}/inventory`,
                ...
                >
                  ...
                  <div className="h-40 sm:h-48 bg-gray-200 rounded-t-lg overflow-hidden relative">
                    {product.images && product.images.length > 0 ? (
                      <img
                        src={`https://agricorus.duckdns.org${product.images[0]}`}
                        alt={product.name}
          ...
          <InventoryUpdateModal
            ...
            onUpdate={async (data) => {
              ...
              try {
                const token = localStorage.getItem("token");
                const res = await axios.patch(
                  `https://agricorus.duckdns.org/api/vendor/products/${selectedProduct._id}/inventory`,
                  data,
                  {
                    headers: { Authorization: `Bearer ${token}` },
                  }
