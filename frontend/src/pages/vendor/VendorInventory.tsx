  ...
  const fetchInventoryData = async () => {
    try {
      ...
      const [overviewRes, alertsRes, movementsRes] = await Promise.all([
        axios.get(
          `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/vendor/inventory`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
        axios.get(
          `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/vendor/inventory/alerts`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
        axios.get(
          `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/vendor/inventory/movements`,
          { headers: { Authorization: `Bearer ${token}` } }
        )
      ]);
  ...
  const handleUpdateStock = async (productId: string) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL || "https://agricorus.duckdns.org"}/api/vendor/inventory/${productId}/stock`,
        {
          stock: newStock,
