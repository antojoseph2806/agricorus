...
const CustomReports = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/analytics/generate-custom-report`,
        reportConfig,
        {
  ...
  const saveReportConfig = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/analytics/save-report-config`,
        reportConfig,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
