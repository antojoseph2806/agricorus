...
const MonthlyReports = () => {
  ...
  const fetchMonthlyReport = async () => {
    try {
      ...
      const response = await axios.get(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/analytics/monthly-report`,
        {
  ...
  const downloadReport = async (format: 'pdf' | 'excel') => {
    try {
      ...
      const response = await axios.get(
        `${(import.meta as any).env.VITE_BACKEND_URL || "http://localhost:5000"}/api/vendor/analytics/download-monthly-report`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: {
            month: selectedDate.getMonth() + 1,
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div>
              ...
              <p className="text-gray-600">Detailed monthly performance analysis and insights.</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
            ...
            <p className="text-gray-500">No sales data found for {currentMonth} {currentYear}</p>
          </div>
        ) : (
          <>
            {/* Download Section */}
            <div className="bg-white rounded-xl shadow-sm border p-6 mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Download Report</h3>
                  <p className="text-gray-600">Export your monthly report in different formats</p>
                </div>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => downloadReport('pdf')}
                    disabled={downloading}
                    className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors duration-300 disabled:opacity-50"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="bg-white rounded-xl shadow-sm border p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`flex items-center gap-1 text-sm ${getGrowthColor(monthlyData.revenueGrowth)}`}>
              ...
              <div className="bg-white rounded-xl shadow-sm border p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`flex items-center gap-1 text-sm ${getGrowthColor(monthlyData.ordersGrowth)}`}>
              ...
              <div className="bg-white rounded-xl shadow-sm border p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`flex items-center gap-1 text-sm ${getGrowthColor(monthlyData.customersGrowth)}`}>
            ...
            {/* Top Products */}
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-6">Top Products This Month</h3>
                    ...
                    <tbody>
                      {monthlyData.topProducts.map((product, index) => (
                        <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-3 px-4 text-right text-gray-600">{product.sold}</td>
