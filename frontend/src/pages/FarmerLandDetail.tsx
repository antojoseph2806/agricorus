...
const FarmerLandDetail: React.FC = () => {
  ...
  // Fetch land details including currentUserLease
  useEffect(() => {
    const fetchLand = async () => {
      try {
        ...
        if (data.currentUserLease) {
          setLeaseStatus(data.currentUserLease.status);
        }
      } catch (err) {
        console.error(err);
      ...
      <div className="max-w-6xl mx-auto px-4">
          ...
          {land.landPhotos && land.landPhotos.length > 0 ? (
            land.landPhotos.map((photo, idx) => (
              <motion.img
                ...
                className="w-full h-72 object-cover rounded-2xl shadow-lg cursor-pointer"
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-4 text-gray-700">
            <p><span className="font-semibold">📍 Address:</span> {land.location.address}</p>
            <p><span className="font-semibold">🌱 Soil Type:</span> {land.soilType}</p>
            <p><span className="font-semibold">💰 Lease Price:</span> ₹{land.leasePricePerMonth.toLocaleString()}/month</p>
            <p><span className="font-semibold">⏳ Duration:</span> {land.leaseDurationMonths} months</p>
            {land.waterSource && <p className="flex items-center gap-2"><Droplet className="w-5 h-5 text-emerald-500" /><span className="font-semibold">Water Source:</span> {land.waterSource}</p>}
            {land.accessibility && <p className="flex items-center gap-2"><Route className="w-5 h-5 text-emerald-500" /><span className="font-semibold">Accessibility:</span> {land.accessibility}</p>}
            {land.sizeInAcres && <p className="flex items-center gap-2"><Ruler className="w-5 h-5 text-emerald-500" /><span className="font-semibold">Size:</span> {land.sizeInAcres} acres</p>}
          </div>
        </motion.div>
        ...
        {land.owner && (
          ...
          >
            ...
            <div className="space-y-2 text-gray-700">
              <p><span className="font-semibold">📧 Email:</span> {land.owner.email}</p>
              <p><span className="font-semibold">📞 Phone:</span> {land.owner.phone}</p>
            </div>
          </motion.div>
        )}
        ...
        >
          {leaseStatus === "pending" && <p className="text-yellow-600 font-medium">Lease request submitted. Awaiting approval.</p>}
          {leaseStatus === "approved" && <p className="text-green-600 font-medium">Lease approved. You have access to this land.</p>}
          {leaseStatus === "cancelled" && <p className="text-red-600 font-medium">Lease request cancelled.</p>}
